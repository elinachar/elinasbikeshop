class CommentUpdateJob < ApplicationJob
  ...
  def perform(comment, current_user)
    ProductChannel.broadcast_to(comment.product_id, comment: render_comment(comment, current_user), average_rating: comment.product.average_rating, comments_size: comment.product.comments.size)
  end
